...
def npy_to_encoded(test_probability, test_id, csv_file, threshold=0.90, min_size=2400):
    ...
    for b in range(num_test):
        ...
        if num_predict > 0:
            ...
            num_predict = 0
            predict = np.zeros((1, 1024, 1024), np.float32)
...
def run_submit():
    ...
    out_dir = \
        '/root/share/project/kaggle/2019/chest/result/deeplab3plus-03-256x256-fold1'
    # '/root/share/project/kaggle/2019/chest/result/deeplab3plus-01-256x256'

    initial_checkpoint = \
        '/root/share/project/kaggle/2019/chest/result/deeplab3plus-03-256x256-fold1/checkpoint/00507500_model.pth'
    ...
    if 1:
        ...
        if mode == 'train':
            ...
            t = (test_truth > 0.5).astype(np.float32)
            p = (test_probability > 0.5).astype(np.float32)
...
def run_analysis():
    ...
    for b in range(len(test_dataset)):
        ...
        probability = test_probability[b, 0]
        predict, num_predict = mask_to_component(
        ...
        print('%5d  %100s  %s   %0.5f   %0.5f' % (
            b, test_id[b], predict.shape, probability.mean(), probability.max()))
        # ---
